...
- transcribe_and_edit_next_by_substr.bat - the same over the whole channel,
  but only for the videos whose title contains the substring set in an
  editable line at the top of the bat; every match is confirmed in turn;
- transcribe_and_edit_next_bypl.bat - the same for one playlist; the
  playlist folder is set in an editable line at the top of the bat
  (default: the real playlist with the most videos);
...
TITLE_SUBSTR_MARKER = "rem TITLE_SUBSTR="
# cmd.exe decodes a bat with the console code page (cp437/cp866/...), which
# mangles a non-ASCII value on its way into a variable. The filter therefore
# stays in a rem line that cmd ignores, and PowerShell reads it back from the
# bat as UTF-8 - no chcp 65001, which would break the console font.
TITLE_SUBSTR_PRELUDE = (
    "$lines = @((Get-Content -LiteralPath '%~f0' -Encoding utf8) "
    f"-match '^{TITLE_SUBSTR_MARKER}'); "
    "if (-not $lines) { "
    "[Console]::WriteLine('ERROR: the TITLE_SUBSTR line is missing'); exit 1 }; "
    f"$SUBSTR = ($lines[0] -replace '^{TITLE_SUBSTR_MARKER}', '').TrimEnd(); "
    "[Console]::WriteLine('Title substring: ' + $SUBSTR); "
    "Add-Content -LiteralPath '%LOGFILE%' -Value ('Title substring: ' + $SUBSTR) "
    "-Encoding utf8; "
)
...
{head_comment}set "CHANNEL={channel}"
set "LANG={lang}"
{head_set}rem ====================================================================
...
{head_echo}call :logecho Language: %LANG% (original only)
call :logblank
...
powershell -NoProfile -Command "[Console]::OutputEncoding = [Text.UTF8Encoding]::new(); {ps_prelude}& %RUNCMD% 2>&1 | ForEach-Object {{ [Console]::WriteLine($_); Add-Content -LiteralPath '%LOGFILE%' -Value $_ -Encoding utf8 }}; exit $LASTEXITCODE"
...
rem LANG is the original language of the videos (two-letter) - used for
rem the transcription cost estimate in the summary.
rem
...
rem Quote the @handle: PowerShell treats bare @ as the splat operator.
set "REFRESHCMD=python -u src\channels\refresh_channel_cache.py '%HANDLE%' --force"
set "SUMMARYCMD=python -u src\channels\get_summary_for_channel.py '%HANDLE%' bypls --lang %LANG% --include-playlist-only"

call :logecho === refresh video cache (force) and rebuild the channel summary ===
call :logecho Log file: %LOGFILE%
call :logecho Channel folder: %CHANNEL%
call :logecho YouTube handle: %HANDLE%
...
call :logecho Results:        _channels\%CHANNEL%\_summaries\%OUTDATE%\
call :logblank
...
def generate_channel_bats(
    ...
    *,
    ...
    substr = DEFAULT_TITLE_SUBSTR.get(lang, DEFAULT_TITLE_SUBSTR_FALLBACK)
    by_substr = BAT_TEMPLATE.format(
        ...
        select_args="--title-substr $SUBSTR --next all",
    ...
    if playlist_folder and (only is None or BYPL_BAT_NAME in only):
        bypl = BAT_TEMPLATE.format(
            ...
            workspace=workspace,
            head_comment=(
                "rem PLAYLIST is the local folder under _playlists/.\n"
                "rem Edit it before a run to switch playlist; default is\n"
                "rem the playlist with the most videos.\n"
            ),
            head_set=f'set "PLAYLIST={playlist_folder}"\n',
            playlist_arg="%PLAYLIST% ",
            head_echo="call :logecho Playlist folder: %PLAYLIST%\n",
            ps_prelude="",
            select_args="--next 1",
    ...
    for name, content, force in (
        ...
        (REFRESH_SUMMARY_BAT_NAME, refresh_summary, False),
    ):
        ...
        if path.exists() and not force:
            ...
            continue
        existed = path.exists()
        # UTF-8 without a BOM: cmd.exe passes such bytes through verbatim, so
        # a Cyrillic SUBSTR reaches Python and the log intact under the OEM
        # console code page - no chcp 65001, which breaks the console font.
        path.write_bytes(content.replace("\n", "\r\n").encode("utf-8"))
        note = (
            f" (PLAYLIST={playlist_folder})" if name == BYPL_BAT_NAME else ""
        )
        if name == REFRESH_SUMMARY_BAT_NAME:
            note = f" (HANDLE={handle})"
        print(
