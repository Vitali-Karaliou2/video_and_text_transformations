...
- create_final_docs.bat - final editing only (no transcription): rebuild
  OUTPUT documents from existing transcripts for the playlist named in
  PLAYLIST (use after archiving bad OUTPUT or to re-edit with --force).
...
Session size is NEXT in each bat's settings file (a number or "all"),
not a flag in the bat itself.
...
CREATE_FINAL_DOCS_BAT_NAME = "create_final_docs.bat"
...
rem
rem Nothing else in this file needs editing. CHANNEL{extra_keys_note} and
rem NEXT live in the sibling settings file next to this bat:
rem
rem     {settings_name}
rem
...
rem Session size (NEXT) comes from the settings file.
set "RUNCMD=python -u src\transcribe\transcribe_videos.py --settings '%SETTINGS%' --lang %LANG% --orig-only --from-youtube --edit --annotate"
...
CREATE_FINAL_DOCS_BAT_TEMPLATE = r"""@echo off
setlocal EnableDelayedExpansion

rem ====================================================================
rem Generated by find_youtube_channel_by_descr.py.
rem
rem Final editing only (no transcription). Rebuilds OUTPUT documents from
rem existing transcripts. Use after moving bad OUTPUT to _archive, or when
rem OUTPUT still exists (--force re-creates it; bad edit cache may still
rem be reused unless INFO/<stem>.edited_sections.json was removed).
rem
rem CHANNEL, PLAYLIST, NEXT and optional VIDEO live in the sibling
rem settings file next to this bat:
rem
rem     {settings_name}
rem
rem They are kept there rather than here because cmd.exe reads a bat in
...
rem --orig-only: original language only; --annotate: 200-250 word
rem annotation .txt; --no-slides: transcript-only videos (remote mode);
rem --force: re-create OUTPUT even when it already exists; --yes: skip
rem per-video cost confirmation. NEXT and VIDEO come from the settings file.
set "RUNCMD=python -u src\docs\create_final_docs.py --settings '%SETTINGS%' --orig-only --annotate --no-slides --force --yes"

call :logecho === final edit only: {scope_note} ===
call :logecho Log file: %LOGFILE%
call :logecho Settings: %SETTINGS%
call :logblank
call :logecho Each video shows its estimated editing cost and waits for
call :logecho a y/n confirmation unless --yes is set in the bat above.
call :logblank
...
def flat_settings_text(channel: str, next_count: str = "1") -> str:
    return (
        ...
        f"CHANNEL={channel}\n"
        "\n"
        "# How many pending videos to process this session (a number, or\n"
        "# 'all' for every pending video).\n"
        f"NEXT={next_count}\n"
        "\n"
...
def by_substr_settings_text(
    channel: str, substr: str, next_count: str = "all"
) -> str:
    return (
        ...
        "\n"
        "# How many matching videos to process this session (a number, or\n"
        "# 'all' for every match).\n"
        f"NEXT={next_count}\n"
        "\n"
        "# Browser whose authorized YouTube cookies yt-dlp should use.\n"
...
def bypl_settings_text(
    channel: str, playlist: str, next_count: str = "1"
) -> str:
    return (
        ...
        "\n"
        "# How many pending videos to process this session (a number, or\n"
        "# 'all' for every pending video in the playlist).\n"
        f"NEXT={next_count}\n"
        "\n"
        "# Browser whose authorized YouTube cookies yt-dlp should use.\n"
        "# Close it completely before running the bat. Chrome does not\n"
...
def create_final_docs_settings_text(
    channel: str, playlist: str, next_count: str = "5"
) -> str:
    return (
        "# Settings of create_final_docs.bat - edit them here and run the bat.\n"
        ...
        f"PLAYLIST={playlist}\n"
        "\n"
        "# How many videos to process this session (ignored when VIDEO is set).\n"
        f"NEXT={next_count}\n"
        "\n"
        "# Optional: process only this video stem (exact filename without\n"
        "# extension). Uncomment and edit to re-edit one video.\n"
        "# VIDEO=01_Introduction [abc123]\n"
    )
...
def next_from_bat_or_settings(
    ...
    *,
    ...
    """Keep a hand-edited NEXT, including the old `--next N` form in the bat."""
    value = value_from_bat_or_settings(
        run_scripts, bat_name, "NEXT", allowed=allowed, fallback=""
    )
    if value:
        return value
    bat_path = run_scripts / bat_name
    ...
    try:
        ...
        return fallback
    match = re.search(r"--next\s+(\w+)", text, re.IGNORECASE)
    if match:
...
def generate_channel_bats(
    ...
    *,
    ...
    / HANDLE / NEXT keep any hand-edited value.
    """
    ...
    substr = value_from_bat_or_settings(
        ...
        BY_SUBSTR_BAT_NAME,
        "TITLE_SUBSTR",
        allowed=("CHANNEL", "TITLE_SUBSTR", "NEXT"),
    ...
    next_flat = next_from_bat_or_settings(
        run_scripts,
        NEXT_BAT_NAME,
        allowed=("CHANNEL", "NEXT"),
        fallback="1",
    )
    next_substr = next_from_bat_or_settings(
        run_scripts,
        BY_SUBSTR_BAT_NAME,
        allowed=("CHANNEL", "TITLE_SUBSTR", "NEXT"),
        fallback="all",
    )
    next_bypl = next_from_bat_or_settings(
        run_scripts,
        BYPL_BAT_NAME,
        allowed=("CHANNEL", "PLAYLIST", "NEXT"),
        fallback="1",
    )
    next_docs = next_from_bat_or_settings(
        run_scripts,
        CREATE_FINAL_DOCS_BAT_NAME,
        allowed=("CHANNEL", "PLAYLIST", "NEXT", "VIDEO"),
        fallback="5",
    )
    ...
    flat_settings = flat_settings_text(channel, next_flat)
    ...
    create_final_docs: str | None = None
    create_final_docs_settings: str | None = None
    create_final_docs_name = "create_final_docs.settings.txt"
    if playlist_value and (only is None or CREATE_FINAL_DOCS_BAT_NAME in only):
        create_final_docs = CREATE_FINAL_DOCS_BAT_TEMPLATE.format(
            workspace=workspace,
            settings_name=create_final_docs_name,
            log_prefix="create_final_docs",
            scope_note="next unedited videos from the playlist named in PLAYLIST",
        )
        create_final_docs_settings = create_final_docs_settings_text(
            channel, playlist_value, next_docs
        )
    elif only is None or CREATE_FINAL_DOCS_BAT_NAME in only:
        print(
            "  WARNING: no playlist folders in _cache/playlists.json; "
            f"{CREATE_FINAL_DOCS_BAT_NAME} was not generated.",
            flush=True,
        )
    ...
    for name, content, settings_body, force in (
        ...
        # on the old rem/set / --next-in-bat scheme so existing channels
        # pick up the change.
        bat_text = ""
        if path.is_file():
            try:
        ...
        if name == BYPL_BAT_NAME:
            note = f" (PLAYLIST={playlist_value}, NEXT={next_bypl})"
        elif name == CREATE_FINAL_DOCS_BAT_NAME:
            note = f" (PLAYLIST={playlist_value}, NEXT={next_docs})"
        elif name == BY_SUBSTR_BAT_NAME:
            note = f" (TITLE_SUBSTR={substr}, NEXT={next_substr})"
        elif name == NEXT_BAT_NAME:
            note = f" (NEXT={next_flat})"
